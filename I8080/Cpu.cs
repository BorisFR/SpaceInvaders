...
namespace eZet.i8080.Emulator
{
	...
	public class Cpu
	{
		...
		internal void run ()
		{
			...
			DateTime begin = DateTime.Now; // = TimeSpan.Zero;
			while (!Halt) {
				if (Debug) {
					Debugger.executing (Reg.Pc);
				}
				if (Throttle) {
					begin = DateTime.Now; // Process.GetCurrentProcess ().TotalProcessorTime;
				}
				...
				if (Throttle && throttleCycle > cyclesPerThrottle) {
					DateTime end = DateTime.Now; // Process.GetCurrentProcess ().TotalProcessorTime;
		...
		{
			if (adr == 0) {
				System.Diagnostics.Debug.WriteLine ("Jmp 0");
			}
			Reg.Pc = adr;
		}
		...
		private void debugPrint (DWord address)
		{
			if (Debug && address == 0x5) {
				if (Reg [SymRef.C] == 9) {
					...
					System.Diagnostics.Debug.WriteLine (Reg [SymRefD.HL]);
				}
			}
		}
